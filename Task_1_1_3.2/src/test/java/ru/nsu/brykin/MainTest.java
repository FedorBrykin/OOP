...
class MainTest {
    ...
    @Test
    public void parceTest4() {
        Parser parser = new Parser();
        Expression e = parser.parse("x * 1");
        final String standardOutput = e.simplify().toString();
        assertTrue(standardOutput.contains("x"));
    }

    @Test
    public void parceTest5() {
        Parser parser = new Parser();
        Expression e = parser.parse("0 * 1");
        final String standardOutput = e.simplify().toString();
        assertTrue(standardOutput.contains("0"));
    }

    @Test
    public void parceTest6() {
        Parser parser = new Parser();
        Expression e = parser.parse("15 - 15");
        final String standardOutput = e.simplify().toString();
        assertTrue(standardOutput.contains("0"));
    }
}